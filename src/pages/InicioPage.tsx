...
const InicioPage: React.FC<InicioPageProps> = ({ currentText, navigateToPage }) => {
  return (
    <section className="pt-14 sm:pt-16 min-h-screen flex items-center justify-center relative overflow-hidden px-4">
      ...
      <div className="absolute inset-0">
        <Feather className="absolute top-16 sm:top-20 left-4 sm:left-10 w-16 h-16 sm:w-32 sm:h-32 text-amber-200/30 animate-float" style={{animationDelay: '0s'}} />
        <BookOpen className="absolute top-32 sm:top-40 right-8 sm:right-20 w-12 h-12 sm:w-24 sm:h-24 text-orange-200/30 animate-float" style={{animationDelay: '1s'}} />
        <FileText className="absolute bottom-24 sm:bottom-32 left-8 sm:left-20 w-14 h-14 sm:w-28 sm:h-28 text-yellow-200/30 animate-float" style={{animationDelay: '2s'}} />
      </div>
      ...
      <div className="relative z-10 text-center max-w-5xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 justify-center items-center mb-12 sm:mb-16 animate-fade-in-up px-4" style={{animationDelay: '0.5s'}}>
          <button
